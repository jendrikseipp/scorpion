...
def get_parser():
    parser = CommonParser()
    # --- C++ translator (translate-cpp) attributes -------------------------
    # Self-reported by the translator's final "Done! [<cpu>s CPU, <wall>s
    # wall-clock]" line and its phase log.
    parser.add_pattern(
        "translator_time", r"Done! \[(.+?)s CPU, .+?s wall-clock\]", type=float
    )
    parser.add_pattern(
        "translator_wall_time",
        r"Done! \[.+?s CPU, (.+?)s wall-clock\]",
        type=float,
    )
    parser.add_pattern(
        "translator_self_memory_kb",
        r"Translator peak memory: (\d+) KB",
        type=int,
    )
    # Peak memory (max RSS) measured externally with /usr/bin/time -v, so it is
    # uniform across revisions and independent of the translator's own
    # accounting. This is the memory attribute the reports use.
    parser.add_pattern(
        "peak_memory_kb",
        r"Maximum resident set size \(kbytes\): (\d+)\n",
        type=int,
    )
    # Output-shape attributes: identical across revisions confirms every
    # optimization stayed byte-equivalent (the whole campaign's invariant).
    parser.add_pattern("translator_task_size", r"Translator task size: (\d+)", type=int)
    parser.add_pattern("translator_variables", r"Translator variables: (\d+)", type=int)
    parser.add_pattern("translator_operators", r"Translator operators: (\d+)", type=int)
    parser.add_pattern("translator_axioms", r"Translator axioms: (\d+)", type=int)

    def set_coverage(content, props):
        # A run "counts" when the translator finished (task size reported).
        props["coverage"] = 1 if "translator_task_size" in props else 0
