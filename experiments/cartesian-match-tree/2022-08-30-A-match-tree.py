...
CONFIGS = [
    (f"{nick}-{transition_representation}", ["--search", f"astar(cegar(subtasks=[original()], max_states=100K, max_transitions=infinity, max_time=infinity, sort_transitions=true, transition_representation={transition_representation}, pick_flawed_abstract_state={pick_state}, pick_split={split}, tiebreak_split={tiebreak_split}, memory_padding=500, random_seed=0, max_concrete_states_per_abstract_state=infinity, max_state_expansions=1M))"])
    for transition_representation in ["ts", "sg"]
...
def clamp_init_h_values(run):
    max_h = 100
    init_h = run.get("initial_h_value")
    if init_h is not None and init_h > max_h:
        run["initial_h_value"] = max_h
    return run

project.add_scatter_plot_reports(
    exp,
    algorithm_pairs=[("greedy", "pairwise-weighted")],
    attributes=["initial_h_value"],
    filter=clamp_init_h_values,
)
