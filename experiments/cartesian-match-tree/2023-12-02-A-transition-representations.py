...
CONFIGS = [
    (f"{nick}{'-cache' if store_spt_children and store_spt_parents else ''}", ["--search", f"astar(cegar(subtasks=[original()], max_states=infinity, max_transitions=infinity, max_time={max_time}, sort_transitions=true, transition_representation={transition_representation}, pick_flawed_abstract_state={pick_state}, pick_split={split}, tiebreak_split={tiebreak_split}, memory_padding=512, random_seed=0, max_concrete_states_per_abstract_state=1K, max_state_expansions=1M, store_shortest_path_tree_children={store_spt_children}, store_shortest_path_tree_parents={store_spt_parents}, use_max=true), verbosity=normal)"])
    for nick, transition_representation in [
        ("01-ts", "ts"),
        ("02-sg", "sg"),
        ("03-ts+sg", "ts_then_sg"),
    ]
    ...
    for store_spt_children, store_spt_parents in ([
        ...
        #(True, False),
        (True, True),
    ] if transition_representation in {"sg", "ts_then_sg"} else [
        (False, False),
    ])
