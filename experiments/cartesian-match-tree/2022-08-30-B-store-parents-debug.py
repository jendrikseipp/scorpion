...
CONFIGS = [
    (f"{nick}-{transition_representation}-children={store_spt_children}-parents={store_spt_parents}", ["--search", f"astar(cegar(subtasks=[original()], max_states=100K, max_transitions=infinity, max_time=infinity, sort_transitions=true, transition_representation={transition_representation}, pick_flawed_abstract_state={pick_state}, pick_split={split}, tiebreak_split={tiebreak_split}, memory_padding=500, random_seed=0, max_concrete_states_per_abstract_state=infinity, max_state_expansions=1M, store_shortest_path_tree_children={store_spt_children}, store_shortest_path_tree_parents={store_spt_parents}), bound=0)"])
