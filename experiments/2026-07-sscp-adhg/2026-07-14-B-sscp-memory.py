...
"""
Confirmation run for the segment-2 memory work of the autoresearch loop
(runs 51-57: budgeted+interned restricted-cost caches, sparse SCFs,
diff-blob cost function registry with chunked pool, 16-byte DAG nodes).

Single revision on all optimal STRIPS benchmarks with the same limits as
2026-07-13-A, so coverage and per-task peaks are directly comparable to
the 01..05 revisions of that experiment (same cluster and partition).
"""
