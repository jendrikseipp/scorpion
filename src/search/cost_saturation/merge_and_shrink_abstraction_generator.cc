#include "merge_and_shrink_abstraction_generator.h"
...
MergeAndShrinkAbstractionGenerator::MergeAndShrinkAbstractionGenerator(const options::Options &opts)
    : pattern_generator(
...
static shared_ptr<AbstractionGenerator> _parse(OptionParser &parser) {
    ...
    return make_shared<MergeAndShrinkAbstractionGenerator>(opts);
}

static PluginShared<AbstractionGenerator> _plugin("merge_and_shrink", _parse);
}
