...
OptimalCostPartitioningHeuristic::OptimalCostPartitioningHeuristic(
    ...
    for (int i = 0; i < static_cast<int>(abstraction_functions.size()); ++i) {
        int init_id =
            abstraction_functions[i]->get_abstract_state_id(initial_state);
...
int OptimalCostPartitioningHeuristic::compute_heuristic(
    const State &ancestor_state) {
    State concrete_state = convert_ancestor_state(ancestor_state);
    // Set upper bound for distance of current abstract states to 0 and for all
    // other abstract states to infinity.
    for (int id = 0; id < static_cast<int>(abstraction_functions.size());
         ++id) {
        int new_state_id =
            abstraction_functions[id]->get_abstract_state_id(concrete_state);
        ...
        lp_solver.set_variable_upper_bound(
            old_state_var, lp_solver.get_infinity());
...
void OptimalCostPartitioningHeuristic::generate_lp(
    ...
    */
    ...
    lp::LinearProgram lp(
        ...
        move(lp_constraints), lp_solver.get_infinity());
...
void OptimalCostPartitioningHeuristic::add_abstraction_variables(
    ...
    double default_lower_bound =
        allow_negative_costs ? -lp_solver.get_infinity() : 0.;
