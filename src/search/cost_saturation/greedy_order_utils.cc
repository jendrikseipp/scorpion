...
int compute_costs_stolen_by_heuristic(
    const vector<int> &saturated_costs, const vector<int> &surplus_costs) {
    assert(saturated_costs.size() == surplus_costs.size());
    int num_operators = surplus_costs.size();
    int sum_stolen_costs = 0;
    for (int op_id = 0; op_id < num_operators; ++op_id) {
        int stolen_costs =
            compute_stolen_costs(saturated_costs[op_id], surplus_costs[op_id]);
...
static int compute_surplus_costs(
    const vector<vector<int>> &saturated_costs_by_abstraction, int op_id,
    int remaining_costs) {
...
vector<int> compute_all_surplus_costs(
    ...
    for (int op_id = 0; op_id < num_operators; ++op_id) {
        surplus_costs.push_back(compute_surplus_costs(
            saturated_costs_by_abstraction, op_id, costs[op_id]));
    }
...
void add_scoring_function_to_feature(plugins::Feature &feature) {
    feature.add_option<ScoringFunction>(
        "scoring_function", "metric for ordering abstractions/landmarks",
