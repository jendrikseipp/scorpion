...
double OrderGeneratorGreedy::rate_abstraction(
    ...
    assert(
        utils::in_bounds(abstract_state_id, h_values_by_abstraction[abs_id]));
...
void OrderGeneratorGreedy::initialize(
    ...
    utils::g_log << "Time for computing h values and saturated costs: " << timer
                 << endl;

    vector<int> surplus_costs =
        compute_all_surplus_costs(costs, saturated_costs_by_abstraction);
    ...
    utils::g_log << "Time for initializing greedy order generator: " << timer
                 << endl;
}
