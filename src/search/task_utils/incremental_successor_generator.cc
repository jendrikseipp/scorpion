...
IncrementalSuccessorGenerator::IncrementalSuccessorGenerator(
    ...
    for (OperatorProxy op : task_proxy.get_operators()) {
        ...
        for (FactProxy precondition : op.get_preconditions()) {
            precondition_of[get_fact_id(precondition.get_pair())].push_back(
                op.get_id());
        }
    ...
    utils::g_log << "Time for initializing marked successor generator: "
                 << init_timer << endl;
}
...
void IncrementalSuccessorGenerator::mark_operator_applicable(int op) {
    ...
    assert(
        find(applicable_operators.begin(), applicable_operators.end(), op) ==
...
void IncrementalSuccessorGenerator::mark_operator_inapplicable(int op) {
    ...
    swap(
        applicable_operators_position[op],
        applicable_operators_position[last_op]);
    assert(applicable_operators.back() == op);
    applicable_operators.pop_back();
    applicable_operators_position[op] = -1;
}

void IncrementalSuccessorGenerator::switch_facts(
    FactPair old_fact, FactPair new_fact) {
...
void IncrementalSuccessorGenerator::push_transition(
    const State &src, int op_id) {
...
void IncrementalSuccessorGenerator::pop_transition(
    const State &src, int op_id) {
    for (FactPair new_fact : effects_by_operator[op_id]) {
        FactPair old_fact = src[new_fact.var].get_pair();
