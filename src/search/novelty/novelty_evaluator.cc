...
void NoveltyEvaluator::notify_initial_state(const State &initial_state) {
    ...
    int novelty = novelty_tables.at(eval_values).compute_novelty_and_update_table(initial_state);
...
void NoveltyEvaluator::notify_state_transition(
    ...
    if (heuristic_cache[state].dirty) {
        ...
        if (it == novelty_tables.end()) {
            it = novelty_tables.emplace_hint(it, eval_values, NoveltyTable(width, task_info));
        }
        int novelty = -1;
...
class NoveltyEvaluatorFeature : public plugins::TypedFeature<Evaluator, NoveltyEvaluator> {
public:
    NoveltyEvaluatorFeature() : TypedFeature("novelty") {
        ...
        document_synopsis(
            ...
            utils::format_conference_reference(
                ...
                "3590-3596",
                "AAAI Press",
                "2017") + "and BFWS was integrated into Scorpion in" +
            utils::format_conference_reference(
    ...
    virtual shared_ptr<NoveltyEvaluator>
    create_component(const plugins::Options &opts) const override {
        int width = opts.get<int>("width");
        int num_vars = TaskProxy(*opts.get<shared_ptr<AbstractTask>>("transform")).get_variables().size();
        if (num_vars > opts.get<int>("max_variables_for_width2")) {
            utils::g_log << "Number of variables exceeds limit " << " --> use width=1" << endl;
            width = 1;
        }
