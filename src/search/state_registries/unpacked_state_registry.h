#ifndef UNPACKED_STATE_REGISTRY_H
#define UNPACKED_STATE_REGISTRY_H
...
/*
  ...
  State
    ...
    Registered states contain a pointer to the UnpackedStateRegistry that created them
  ...
  UnpackedStateRegistry
    The UnpackedStateRegistry allows to create states giving them an ID. IDs from
    different state registries must not be mixed.
    The UnpackedStateRegistry also stores the actual state data in a memory friendly way.
  ...
  PerStateInformation<T>
    Associates a value of type T with every state in a given UnpackedStateRegistry.
...
class UnpackedStateRegistry :
    ...
    struct StateIDSemanticHash {
        const segmented_vector::SegmentedArrayVector<unsigned> &state_data_pool;
        int state_size;
        StateIDSemanticHash(
            const segmented_vector::SegmentedArrayVector<unsigned> &state_data_pool,
            int state_size)
        ...
        uint64_t operator()(int id) const {
            const unsigned *data = state_data_pool[id];
    ...
    struct StateIDSemanticEqual {
        ...
        bool operator()(int lhs, int rhs) const {
            const unsigned *lhs_data = state_data_pool[lhs];
            const unsigned *rhs_data = state_data_pool[rhs];
    ...
    */
    ...
    segmented_vector::SegmentedArrayVector<unsigned> state_data_pool;
