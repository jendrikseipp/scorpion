#include "fixed_tree_packed_state_registry.h"
...
FixedTreePackedStateRegistry::FixedTreePackedStateRegistry(const TaskProxy &task_proxy)
    ...
    State::get_variable_value = [this](const StateID& id) {
            ...
            vst::read_state(id.value, get_bins_per_state(), tree_table, buffer);
...
StateID FixedTreePackedStateRegistry::insert_id_or_pop_state() {
    /*
...
State FixedTreePackedStateRegistry::lookup_state(StateID id) const {
    std::vector<vs::Index> buffer(get_bins_per_state());
    vst::read_state(id.value, get_bins_per_state(), tree_table, buffer);
...
State FixedTreePackedStateRegistry::lookup_state(
    StateID id, vector<int> &&state_values) const {
    return task_proxy.create_state(*this, id, move(state_values));
}

const State &FixedTreePackedStateRegistry::get_initial_state() {
...
State FixedTreePackedStateRegistry::get_successor_state(const State &predecessor, const OperatorProxy &op) {
    assert(!op.is_axiom());
...
int FixedTreePackedStateRegistry::get_state_size_in_bytes() const {
    return get_bins_per_state() * sizeof(unsigned);
}

int FixedTreePackedStateRegistry::get_bins_per_state() const {
    return state_packer.get_num_bins();
}
void FixedTreePackedStateRegistry::print_statistics(utils::LogProxy &log) const {
