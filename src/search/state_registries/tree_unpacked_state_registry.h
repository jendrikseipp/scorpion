...
/*
  ...
  State
    ...
    Registered states contain a pointer to the TreeUnpackedStateRegistry that created them
  ...
  TreeUnpackedStateRegistry
    The TreeUnpackedStateRegistry allows to create states giving them an ID. IDs from
    different state registries must not be mixed.
    The TreeUnpackedStateRegistry also stores the actual state data in a memory friendly way.
  ...
  PerStateInformation<T>
    Associates a value of type T with every state in a given TreeUnpackedStateRegistry.
