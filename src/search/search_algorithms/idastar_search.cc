...
FifoCache::FifoCache(int max_size) : max_size(max_size) {
    if (max_size == INF) {
    ...
    for (OperatorID op_id : applicable_ops) {
        ...
        if (cache) {
            ...
            if (succ_g > old_succ_g ||
                (succ_g == old_succ_g && iteration == old_iteration)) {
                ++num_cache_hits;
                continue;
            } else {
...
SearchStatus IDAstarSearch::step() {
    ...
    while (f_limit != INF && f_limit != -1 &&
           (!single_plan || !found_solution())) {
...
public:
    IDAstarSearchFeature() : TypedFeature("idastar") {
        ...
        add_option<int>(
            "initial_f_limit", "initial depth limit", "0",
        ...
        add_option<bool>(
            "single_plan", "stop after finding the first plan", "true");
