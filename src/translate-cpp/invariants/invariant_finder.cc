...
int BalanceChecker::next_index(size_t upper_bound) {
    ...
    uniform_int_distribution<int> dist(0, static_cast<int>(upper_bound) - 1);
    return dist(random_);
}
...
vector<const Predicate *> get_fluents(const Task &task) {
    ...
    for (const auto &p : task.predicates)
        if (fluent_names.contains(p.name))
            out.push_back(&p);
    return out;
}
...
vector<Invariant> initial_invariants(const Task &task, int limit) {
    ...
    for (const auto *p : fluents) {
        ...
        for (size_t omitted = 0; omitted < p->arguments.size(); ++omitted) {
            vector<int> inv_args;
            for (size_t i = 0; i < omitted; ++i)
                inv_args.push_back(static_cast<int>(i));
            inv_args.push_back(COUNTED);
            for (size_t i = omitted; i + 1 < p->arguments.size(); ++i)
                inv_args.push_back(static_cast<int>(i));
            if (static_cast<int>(result.size()) >= limit)
                break;
...
vector<Invariant> find_invariants(
    ...
    unordered_set<Invariant, InvariantHash> seen(
        initial.begin(), initial.end());
    BalanceChecker checker(task, reachable_action_parameters);

    auto enqueue = [&](Invariant inv) {
        if (static_cast<int>(seen.size()) >= limit)
            return;
        if (seen.insert(inv).second)
            queue.push_back(move(inv));
    };
    ...
    while (!queue.empty()) {
        ...
        if (t.seconds() > opts.invariant_generation_max_time) {
            cout << "Time limit reached, aborting invariant generation" << endl;
            break;
        }
...
vector<vector<ConditionPtr>> get_groups(
    ...
    // Group by predicate.
    unordered_map<string, vector<const Invariant *>> predicate_to_inv;
    ...
    struct GroupKeyHash {
        size_t operator()(const GroupKey &k) const noexcept {
            ...
            for (const auto &s : k.second)
                h ^= hash<string>{}(s) + 0x9e3779b9u + (h << 6) + (h >> 2);
            return h;
        }
    };
