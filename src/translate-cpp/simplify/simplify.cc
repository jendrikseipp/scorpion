...
vector<DTG> build_dtgs(const SASTask &task) {
    ...
    auto effective_pre = [](int var_no,
                            const unordered_map<int, int> &conds,
                            const vector<VarVal> &eff_cond)
        -> optional<int> {
        ...
        for (const auto &[cv, cval] : eff_cond) {
            if (cv == var_no) {
                ...
                else if (cval != result) return nullopt;
            }
        }
        return result;
    };
...
optional<SASOperator> translate_operator(const Renaming &r,
    ...
    new_pre_post.erase(unique(new_pre_post.begin(), new_pre_post.end()),
                       new_pre_post.end());
    SASOperator out;
    out.name = op.name;
...
void filter_unreachable_propositions(SASTask &task) {
    ...
    for (auto &op : task.operators) {
        ...
        if (nop) new_ops.push_back(move(*nop));
        else ++removed;
    }
