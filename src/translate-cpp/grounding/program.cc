...
ostream &operator<<(ostream &os, const Atom &a) {
    ...
    for (size_t i = 0; i < a.args.size(); ++i) {
        if (i)
            os << ", ";
        if (a.args[i].is_symbol())
            os << a.args[i].name();
        else
            os << a.args[i].position();
    }
    os << ")";
    return os;
}
...
bool Rule::rename_duplicate_variables() {
    ...
    for (auto &cond : conditions)
        rename(cond, extra);
    for (auto &c : extra)
        conditions.push_back(move(c));
    return !extra.empty();
}

namespace {
bool has_unbound_effect_vars(const Rule &r, unordered_set<string> &out) {
...
void Program::normalize() {
    ...
    for (auto &r : rules) {
        ...
        if (has_unbound_effect_vars(r, unbound)) {
            ...
            vector<string> sorted_unbound(unbound.begin(), unbound.end());
    ...
    for (auto &r : rules) {
        if (r.rename_duplicate_variables() && !printed) {
            cout << "Duplicate arguments: Adding equality conditions." << endl;
            printed = true;
        }
    }
