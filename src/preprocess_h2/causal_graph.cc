...
// This method is called after deriving mutexes and unreachable atoms.
...
void CausalGraph::update() {
    cout << "Updating causal graph and pruning unnecessary atoms" << endl;
    ...
    for (int i = 0; i < old_size; i++) {
        if (ordering[i]->is_necessary() || g_do_not_prune_variables) {
            ...
            ordering[i]->remove_unreachable_atoms();
        }
    }
    ...
    // Put -1 to every variable before calculating important vars
...
void CausalGraph::get_strongly_connected_components(
    ...
    for (const auto &[var, weighted_succ] : weighted_graph) {
        int index = variableToIndex[var];
        vector<int> &succ = unweighted_graph[index];
        for (const auto &[succ_var, weight] : weighted_succ)
            succ.push_back(variableToIndex[succ_var]);
    }
...
void CausalGraph::calculate_important_vars() {
    unordered_set<Variable *> necessary_vars;

    // Mark all variables reachable from goals via DFS
    for (const auto &[var, val] : goals) {
        dfs(var, necessary_vars);
    }
    ...
    for (Variable *var : ordering) {
        if (necessary_vars.contains(var) || g_do_not_prune_variables) {
...
void CausalGraph::dfs(
    Variable *from, unordered_set<Variable *> &necessary_vars) {
    if (necessary_vars.contains(from)) {
        return; // Already visited
    }
    necessary_vars.insert(from);

    for (const auto &[curr_predecessor, weight] : predecessor_graph[from]) {
        dfs(curr_predecessor, necessary_vars);
    }
}
...
void CausalGraph::dump() const {
    for (const auto &[var, successors] : weighted_graph) {
        cout << "dependent on var " << var->get_name() << ": " << endl;
        for (const auto &[succ_var, weight] : successors) {
            cout << "  [" << succ_var->get_name() << ", " << weight << "]"
                 << endl;
        }
    }
    for (const auto &[var, predecessors] : predecessor_graph) {
        cout << "var " << var->get_name() << " is dependent of: " << endl;
        for (const auto &[pred_var, weight] : predecessors)
            cout << "  [" << pred_var->get_name() << ", " << weight << "]"
                 << endl;
    }
}
void CausalGraph::generate_cpp_input(
    ...
    for (const auto &[source_var, successors] : weighted_graph) {
        if (source_var->get_level() != -1) {
            // source variable influences others
            WeightedSuccessors &curr = (WeightedSuccessors &)successors;
            succs[source_var->get_level()] = &curr;
            // count number of influenced vars
            int num = 0;
            for (const auto &[succ_var, weight] : curr)
                if (succ_var->get_level() != -1
                    // && succ_var->get_level() > source_var->get_level()
                )
                    num++;
    ...
    for (int i = 0; i < num_vars; i++) {
        ...
        for (const auto &[succ_var, weight] : *curr) {
            if (succ_var->get_level() != -1
                // && succ_var->get_level() > ordered_vars[i]->get_level()
            )
                // the variable succ_var is important and influenced by
                // variable i print level and weight of influence
                outfile << succ_var->get_level() << " " << weight << endl;
        }
    }
}
